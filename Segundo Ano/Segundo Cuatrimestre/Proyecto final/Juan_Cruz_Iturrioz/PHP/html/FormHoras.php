	...
	<form action="" method="post">
		...
		<select name = "Horario_de_embarcacion" id = "Horario_de_embarcacion" onclick="myFunction(1)">
			...
			<?php foreach ($this->Viajes as $V ) { ?>
				<option value="<?=$this->ORI?>::<?=$this->DES?>::<?= $this->FEC?>::<?= $V['Horario_de_embarcacion']->format('H:i')?>::<?= $V['Horario_de_llegada']->format('H:i') ?>"> 
					<?= $V['Horario_de_embarcacion']->format('H:i') ?> </option>						
		...
		<select name = "Horario_de_llegada" id = "Horario_de_llegada" onclick="myFunction(2)" >
			...
			<?php foreach ($this->Viajes as $V ) { ?>
				<option value="hola mundo"> 
					<?= $V['Horario_de_llegada']->format('H:i') ?> </option>						
